...
export const getStudentById = asyncHandler<Student | null>(
  async (req, _res, _next) => {
    ...
    if (!data) return { status: 404, message: "Student not found", data: null, error: "Not found" }
    return { status: 200, message: "Student fetched", data, error: null }
  },
)

export const createStudent = asyncHandler<Student>(
  async (req, _res, _next) => {
    const doc = await StudentModel.create(req.body)
    return { status: 201, message: "Student created", data: doc.toObject(), error: null }
  },
)
...
export const deleteStudent = asyncHandler<null>(
  async (req, _res, _next) => {

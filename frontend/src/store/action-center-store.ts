...
import type { Student, Task, TaskStatus, Message } from "@zyra-ass/shared"
...
export const useActionCenterStore = create<ActionCenterState>((set, get) => ({
    student: null,
    tasks: [],
    messages: [],
    loading: false,
    ...
    fetchStudent: async (id) => {
        ...
        try {
            const student = await fetcher<Student>(`/students/${id}`)
    ...
    fetchTasks: async (studentId) => {
        try {
            const tasks = await fetcher<Task[]>(`/students/${studentId}/tasks`)
            set({ tasks })
        } catch {
    ...
    refreshAll: async (studentId) => {
        ...
        try {
            const [student, tasks, messages] = await Promise.all([
                ...
                fetcher<Message[]>(`/students/${studentId}/messages`),
            ])
            set({
                student,
                tasks,
    ...
    updateTask: async (taskId, data) => {
        const prev = get().tasks
        set({
            tasks: prev.map((t) => (t.id === taskId ? { ...t, ...data } : t)),
        })
        try {

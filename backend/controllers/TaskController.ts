...
import type { ApiResponse, Task } from "@zyra-ass/shared"

export const getTasks = asyncHandler<Task[]>(
...
export const getTaskById = asyncHandler<Task | null>(
  async (req, _res, _next) => {
    ...
    if (!data) return { status: 404, message: "Task not found", data: null, error: "Not found" }
    return { status: 200, message: "Task fetched", data, error: null }
  },
)

export const createTask = asyncHandler<Task>(
  async (req, _res, _next) => {
    const doc = await TaskModel.create(req.body)
    return { status: 201, message: "Task created", data: doc.toObject(), error: null }
  },
)
...
export const deleteTask = asyncHandler<null>(
  async (req, _res, _next) => {
